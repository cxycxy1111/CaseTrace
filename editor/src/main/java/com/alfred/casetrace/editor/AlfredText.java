...
public class AlfredText extends EditText implements TextWatcher {
    ...
    public AlfredText(Context context) {
        super(context);
        init(null);
    }

    public AlfredText(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public AlfredText(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @SuppressWarnings("NewApi")
    public AlfredText(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(context, attrs, defStyleAttr, defStyleRes);
        init(attrs);
    }

    private void init(AttributeSet attrs) {
        TypedArray array = getContext().obtainStyledAttributes(attrs, R.styleable.AlfredText);
        bulletColor = array.getColor(R.styleable.AlfredText_bulletColor, 0);
        bulletRadius = array.getDimensionPixelSize(R.styleable.AlfredText_bulletRadius, 0);
        bulletGapWidth = array.getDimensionPixelSize(R.styleable.AlfredText_bulletGapWidth, 0);
        historyEnable = array.getBoolean(R.styleable.AlfredText_historyEnable, true);
        historySize = array.getInt(R.styleable.AlfredText_historySize, 100);
        linkColor = array.getColor(R.styleable.AlfredText_linkColor, 0);
        linkUnderline = array.getBoolean(R.styleable.AlfredText_linkUnderline, true);
        quoteColor = array.getColor(R.styleable.AlfredText_quoteColor, 0);
        quoteStripeWidth = array.getDimensionPixelSize(R.styleable.AlfredText_quoteStripeWidth, 0);
        quoteGapWidth = array.getDimensionPixelSize(R.styleable.AlfredText_quoteCapWidth, 0);
        array.recycle();
    ...
    protected void styleInvalid(int style, int start, int end) {
        ...
        List<AlfredPart> list = new ArrayList<>();

        for (StyleSpan span : spans) {
            if (span.getStyle() == style) {
                list.add(new AlfredPart(getEditableText().getSpanStart(span), getEditableText().getSpanEnd(span)));
                getEditableText().removeSpan(span);
            }
        }

        for (AlfredPart part : list) {
            if (part.isValid()) {
    ...
    protected void bulletValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (bulletStart < bulletEnd) {
                getEditableText().setSpan(new AlfredBulletSpan(bulletColor, bulletRadius, bulletGapWidth), bulletStart, bulletEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    protected void quoteValid() {
        ...
        for (int i = 0; i < lines.length; i++) {
            ...
            if (quoteStart < quoteEnd) {
                getEditableText().setSpan(new AlfredQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), quoteStart, quoteEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
    }
    ...
    // When AlfredText lose focus, use this method
    ...
    protected void linkValid(String link, int start, int end) {
        ...
        linkInvalid(start, end);
        getEditableText().setSpan(new AlfredURLSpan(link, linkColor, linkUnderline), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
    }
    ...
    public void fromHtml(String source) {
        ...
        builder.append(AlfredParser.fromHtml(source));
        switchToAlfredStyle(builder, 0, builder.length());
        setText(builder);
    }
    ...
    protected void switchToAlfredStyle(Editable editable, int start, int end) {
        ...
        for (BulletSpan span : bulletSpans) {
            ...
            editable.setSpan(new AlfredBulletSpan(bulletColor, bulletRadius, bulletGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (QuoteSpan span : quoteSpans) {
            ...
            spanEnd = 0 < spanEnd && spanEnd < editable.length() && editable.charAt(spanEnd) == '\n' ? spanEnd - 1 : spanEnd;
            editable.removeSpan(span);
            editable.setSpan(new AlfredQuoteSpan(quoteColor, quoteStripeWidth, quoteGapWidth), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
        ...
        for (URLSpan span : urlSpans) {
            ...
            int spanEnd = editable.getSpanEnd(span);
            editable.removeSpan(span);
            editable.setSpan(new AlfredURLSpan(span.getURL(), linkColor, linkUnderline), spanStart, spanEnd, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
